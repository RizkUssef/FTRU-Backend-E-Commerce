...
@section('content')
    ...
    <section class="all_categories">
        ...
        <div class="add_subcate">
            <a href="{{route('Add new CS to product',["category_id"=>encrypt($category->id),"subcategory_id"=>encrypt($subcategory->id),"product_id"=>encrypt($product->id)])}}">ADD COLOR & SIZE</a>
        </div>
        <div class="all_cate_blocks">
            <div class="cate one_prod">
                <div class="id_link">
                    <a href="#" disabled>{{"# ".$product->id}}</a>
                </div>
                <div class="left">
            ...
            @foreach ($product->productSize as $item)
                @foreach ($item->sizeColor as $quantity)
                        @php
                            ...
                            $color = DB::table('product_colors')->where('id',$colorId)->first();
                        @endphp
                        ...
                        @if ($product->image != $image)
                            <div class="sub_product_container">
                                ...
                                <div class="options">
                                    <a href="#"><img src="{{asset('img/dashboard/icons/edit.png')}}" alt="not" srcset=""></a>
                                    <form action="" method="post">
                                        <button type="submit"><img src="{{asset('img/dashboard/icons/delete.png')}}" alt="not" srcset=""></button>
                                    </form>
                                </div>
                            </div>
                        @endif
